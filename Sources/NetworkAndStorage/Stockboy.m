    ...
    @try
    {
	  ...
	  // just got is 5 and we have the old style private key in the keychain.
		...
		if ([[CryptoUtils getManager] storageVersion] == 5 && [[CryptoUtils getManager] privateKey] != NULL)
		{
			...
			NSLocalizedString(@"Due to a recent update, you need to log in to Firefox Home again.",
				"Due to a recent update, you need to log in to Firefox Home again."), @"message", nil];
